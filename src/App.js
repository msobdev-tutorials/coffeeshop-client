...
import {SecureRoute, Security, withAuth, ImplicitCallback} from "@okta/okta-react";
...
const AuthWrapper = withAuth(class WrappedRoutes extends Component {
    ...
    render() {
        ...
        const navbar = <NavBar isAuthenticated={authenticated} login={this.login.bind(this)} logout={this.logout.bind(this)}/>
        return (
            <Switch>
                <Route path='/' exact={true}
                       render={(props) => <Home {...props} authenticated={authenticated} user={user} api={api} navbar={navbar}/>}
                />
                <SecureRoute path='/coffee-shops' exact={true}
                       render={(props) => <CoffeeShopsList {...props} authenticated={authenticated} user={user} api={api} navbar={navbar}/>}
                />
                <SecureRoute path='/coffee-shops/:id'
                       render={(props) => <CoffeeShopsEdit {...props} authenticated={authenticated} user={user} api={api} navbar={navbar}/>}
                />
            </Switch>
        )
    }
...
class App extends Component {
    ...
    render() {
        return (
            <Router>
                <Security issuer='https://dev-460228.okta.com/oauth2/default'
                          clientId='0oaauyq3pBG0jZgst4x6'
